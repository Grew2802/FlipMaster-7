...
export const Calculator: React.FC<CalculatorProps> = ({ mode, onClose, onSubmit, playerName, theme, toggleTheme }) => {
  ...
  const allModifierCards = useMemo(() => {
    switch (mode) {
      case GameMode.ORIGINAL:
        return MODIFIERS_ORIGINAL;
      case GameMode.VENGEANCE:
        return MODIFIERS_VENGEANCE;
      case GameMode.COMBO:
        return [...MODIFIERS_ORIGINAL, ...MODIFIERS_VENGEANCE];
      default:
        return [];
    }
  }, [mode]);
    ...
    <div className="flex flex-col h-[100dvh] w-full overflow-hidden p-1 bg-bg transition-colors duration-300">
      ...
      <div className="flex-1 min-h-0 overflow-y-auto p-2">
        <div className="w-full max-w-md mx-auto">
          ...
          {allModifierCards.length > 0 && (
            <div className="grid grid-cols-5 gap-2 mt-4">
              {allModifierCards.map((mod, idx) => {
                if (mod.special === 'x2') {
                  return (
                    <Card 
                      key={`mod-x2-${idx}`} 
