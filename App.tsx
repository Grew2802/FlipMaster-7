...
const App = () => {
  ...
  const [goalScore, setGoalScore] = useState(200);
  ...
  useEffect(() => {
    ...
    if (saved) {
      try {
        ...
        setGoalScore(data.goalScore || 200);
      } catch (e) {
  ...
  useEffect(() => {
    localStorage.setItem('flip7_v_data', JSON.stringify({
      players, gameMode, history, theme, sound, goalScore
    }));
    // Apply theme
    if (theme === 'light') {
  ...
  const handleCalcSubmit = (total: number, bonus: number, isF7: boolean, isX2: boolean, isDiv2: boolean, isZeroRule: boolean) => {
  ...
  if (screen === 'title') {
    return (
      <div className="min-h-screen flex flex-col items-center justify-center p-6 text-center space-y-6 relative bg-bg transition-colors duration-300">
        ...
        <SettingsModal isOpen={showSettings} onClose={() => setShowSettings(false)} theme={theme} toggleTheme={toggleTheme} sound={sound} toggleSound={() => setSound(!sound)} goalScore={goalScore} setGoalScore={setGoalScore} />
  ...
  return (
    <div className="min-h-screen flex flex-col p-4 max-w-lg mx-auto bg-bg transition-colors duration-300">
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/90 backdrop-blur-md animate-in fade-in duration-500">
            <div className="text-center p-8">
                ...
                <div className="flex flex-col gap-3">
                    ...
                    <button onClick={() => { setWinner(null); setScreen('stats'); }} className="px-8 py-4 bg-white/10 text-white font-black rounded-2xl border border-white/10 uppercase text-xs tracking-widest">History</button>
                </div>
            </div>
        </div>
      )}
      ...
      <div className="flex-1 space-y-3 overflow-y-auto pb-32">
        {players.map((p, idx) => (
          <div key={p.id} className={`relative transition-all duration-300 ${p.busted ? 'opacity-30 grayscale scale-95' : ''}`}>
             <div className={`p-4 rounded-2xl border transition-colors shadow-sm ${p.score >= Math.max(...players.map(pl=>pl.score)) && p.score > 0 ? 'border-primary bg-primary/5' : 'border-white/10 bg-card-bg'}`}>
                <div className="flex justify-between items-center mb-3">
                   <div className="flex items-center gap-3">
                      ...
                      <div>
                         <div className="font-black text-text leading-none uppercase tracking-tight">{p.name}</div>
                         <div className="text-4xl font-black text-primary leading-none mt-2">{p.score}</div>
                         <div className="text-[10px] text-subtext font-black uppercase tracking-wider mt-1">
                            {goalScore - p.score > 0 ? `${goalScore - p.score} to win` : 'WINNER!'}
                         </div>
                      </div>
                   </div>
                   <div className="flex flex-col gap-2 items-end">
                      ...
                      <button onClick={() => setCalcTarget(idx)} disabled={p.busted} className={`flex items-center gap-2 px-3 py-2 rounded-xl border min-w-[85px] justify-center transition-all ${p.isZeroRule ? 'bg-bust/20 border-bust animate-pulse' : 'bg-white/5 border-white/10 hover:border-primary/50'}`}>
                         {p.currentInput ? <span className="font-black text-text text-xl">{p.currentInput}</span> : <span className="text-subtext text-[9px] font-black uppercase tracking-wider">Score It</span>}
                      </button>
                   </div>
                </div>
      ...
      <div className="fixed bottom-0 left-0 right-0 p-4 bg-gradient-to-t from-bg via-bg to-transparent pointer-events-none">
      ...
      {calcTarget !== null && (
          <div className="fixed inset-0 z-[100] bg-bg flex flex-col animate-in slide-in-from-bottom duration-300 h-[100dvh]">
              <Calculator mode={gameMode} playerName={players[calcTarget].name} onClose={() => setCalcTarget(null)} onSubmit={handleCalcSubmit} theme={theme} toggleTheme={toggleTheme} />
          </div>
      )}
    </div>
  );
